...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    TweetReader.allTweets.filter(x => isContained(x, google))
  }

  def isContained(tweet: Tweet, list: List[String]): Boolean = {
    list.exists(listElement => {
      listElement.contains(tweet.text)
    })
  }


  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(x => isContained(x, apple))
  }
